...
function sanitizeDetectionResult(raw: Partial<AIDetectionResult> | null | undefined): AIDetectionResult {
  if (!raw) return FALLBACK_DETECTION;
  return {
    type: raw.type || FALLBACK_DETECTION.type,
    priority: raw.priority && ['low', 'medium', 'high', 'critical'].includes(raw.priority) ? raw.priority : FALLBACK_DETECTION.priority,
    victimCount: Number.isFinite(raw.victimCount) ? Math.max(0, Math.floor(raw.victimCount)) : FALLBACK_DETECTION.victimCount,
    ...
    priorityScore: Number.isFinite(raw.priorityScore) ? Math.max(0, Math.min(100, raw.priorityScore)) : FALLBACK_DETECTION.priorityScore,
    crowdDensity: raw.crowdDensity && ['low', 'medium', 'high'].includes(raw.crowdDensity) ? raw.crowdDensity : FALLBACK_DETECTION.crowdDensity,
    hazards: Array.isArray(raw.hazards) ? raw.hazards.filter((item): item is string => typeof item === 'string') : [],
    missingPersonDetected: Boolean(raw.missingPersonDetected),
  };
}

export async function analyzeDroneFeed(base64Image: string, audioTranscript?: string): Promise<AIDetectionResult> {
  if (!ai || !base64Image) {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            crowdDensity: { type: Type.STRING, enum: ["low", "medium", "high"] },
            hazards: { type: Type.ARRAY, items: { type: Type.STRING } },
            missingPersonDetected: { type: Type.BOOLEAN }
          },
          required: ["type", "priority", "victimCount", "description", "keywords", "priorityScore", "crowdDensity", "hazards", "missingPersonDetected"]
        },
      }
    });
...
export async function predictDisasterTrend(historicalTasks: Task[]): Promise<string> {
